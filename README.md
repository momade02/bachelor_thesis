...
**Predicted returns track well but investable clones do not.** Out-of-sample predicted returns
(intercept included) correlate 0.70+ with realised returns for nine of ten strategies, and
no paired t-test rejects equality of means. Once the intercept is dropped (manager
...
| File | Description |
| --- | --- |
| `Bachelor_Thesis_Moritz_Maidl.pdf` | Full thesis, including appendix with all regression tables and plots. |
...
**Note on padded series.** Several ETFs did not trade for the whole window, so I
zero-filled their leading months: `VIXY` (first 14), `RSLC` and `SIZE` (first 16), `PHDG`
(first 37), `MTUM` (first 41). Zeros are not missing-data indicators here as they enter the
regressions as genuine zero returns and bias early-window estimates toward zero. I document this in the 
...
*(t−60)* to *(t−1)* and applied to month *t*'s factor realisations. Rolling windows instead of
fixed weights, because fixed weights impose look-ahead bias and cannot adapt to the
exposure drift. This yields **105 out-of-sample months** (months
...
Full tables are in the thesis appendix (Tables 3–5).
...
of the nine factors, out-of-sample correlation of 0.16. Two possible readings: the strategy really is
market-neutral on average, or a linear model with these factors simply cannot decompose
returns generated by systematic, quantitatively driven programmes. I do not
separate the two in the thesis.
...
**Warning:** `Skript.R` is a single 3,138-line exploratory script written to be
...
off the console and the viewer pane. Executing it end-to-end will open dozens of viewer tabs
and print a lot of output.
...
I did this thesis before I had much data science or software engineering
...
sit near zero. I offer two explanations in the thesis: the yield-versus-price argument from
Cherian et al. (2020), and OLS levering a near-flat series to make it useful. It is
explicit that the second is a guess. Constraining betas to sum to one *inside* the
regression, would avoid the problem the clones ran into.

**No formal OLS diagnostics.** Seven Gauss-Markov assumptions are listed and only
...
of index names. Same for the correlation and performance tests.

**Fees are not modelled.** Index returns are net of fees and factor returns are gross. The
clones' cost advantage which is a main argument for replication in the first place is only asserted.
